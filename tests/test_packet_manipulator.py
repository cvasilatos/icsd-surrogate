...
from proteus.utils.packet_manipulator import construct_prefix, inject_mutation
...
class TestConstructPrefix:
    ...
    def test_returns_empty_when_stop_at_first_field(self) -> None:
        ...
        result = construct_prefix(fields, stop_at_name="f1")
        assert result == b""
    ...
    def test_includes_fields_before_stop(self) -> None:
        ...
        result = construct_prefix(fields, stop_at_name="f3")
    ...
    def test_returns_all_when_stop_name_absent(self) -> None:
        ...
        result = construct_prefix(fields, stop_at_name="nonexistent")
        assert result == bytes.fromhex("aabb")

    def test_returns_empty_for_empty_fields(self) -> None:
        result = construct_prefix([], stop_at_name="f1")
        assert result == b""
...
class TestInjectMutation:
    ...
    def test_replaces_single_byte(self) -> None:
        ...
        result = inject_mutation(target, base, "ff", unique_fields=[target])
        assert result[1] == 0xFF

    def test_replaces_multi_byte(self) -> None:
        base = "0102030405"
        target = _make_field("f", "0203", 1, 2)
        result = inject_mutation(target, base, "aabb", unique_fields=[target])
    ...
    def test_does_not_modify_other_bytes_without_adapter(self) -> None:
        ...
        result = inject_mutation(target, base, "cc", unique_fields=[target])
